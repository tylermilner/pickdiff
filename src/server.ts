...
function createApp(git: SimpleGit, repoPath: string): Express {
  ...
  app.post('/api/diff', async (req: Request, res: Response) => {
    const { startCommit, endCommit, files }: { startCommit: string; endCommit: string; files: string[] } = req.body;
    ...
    try {
      ...
      for (const file of files) {
        let diff: string = await git.diff([`${startCommit}..${endCommit}`, '--', file]);
      ...
      res.status(500).json({ error: error instanceof Error ? error.message : 'Failed to get diffs' });
    }
  });
